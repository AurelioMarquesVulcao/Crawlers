...
class CotaController {
  ...
  static async setValor(req, res) {
    ...
    console.log(cotaEstado)

    if (!cotaEstado)
      res.status(200).send({sucesso: false, detalhes: 'Não foi encontrado processo a uf especificada'});


    await CotaEstado.updateOne({_hash: cotaEstado._hash}, {$set: {ValorCota: valor}})

    res.status(200).send({sucesso: true, detalhes: 'Valor do estado atualizado'})
  }
  ...
  static async getCota(req, res) {
    ...
    if (!cotaEstado)
      res.status(200).send({sucesso: false, detalhes: 'Estado não foi localizado'});
    ...
    let cotaMensais = await CotaMensal.find({_hash: {$in: cotasMensaisHash}})
