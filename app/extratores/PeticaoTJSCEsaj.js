...
class PeticaoTJSCEsaj extends ExtratorPuppeteer {
  constructor({
                url = '',
                debug = false,
                timeout = 30000,
  ...
  async extrair(numeroProcesso, instancia = 1) {
    ...
    this.logger = new Logger('info', 'logs/TJSC/peticao_esaj.log', {
    ...
    try {
      ...
      await this.acessar(
        'https://esaj.tjsc.jus.br/esaj/portal.do',
        this.pageOptions
      );
  ...
  async login() {
    ...
    await new Promise(async (resolve) => {
      do {
        await this.acessar(
          'https://esaj.tjsc.jus.br/sajcas/login?service',
          this.pageOptions,
          false
        );
  ...
  async consultarProcesso(numeroProcesso, instancia = 1) {
    ...
    numeroProcesso = numeroProcesso.replace('.8.24.', '.');
    ...
    if (instancia === 2) {
      url = 'https://esaj.tjsc.jus.br/cposg/open.do';
    }
    if (instancia === 3) {
      url = 'https://esaj.tjsc.jus.br/cposgcr/open.do';
    }
...
module.exports.PeticaoTJSCEsaj = PeticaoTJSCEsaj;
