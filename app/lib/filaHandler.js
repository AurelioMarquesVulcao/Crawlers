...
class GerenciadorFila {
  ...
  */
  constructor(prefetch=1) {
    this.host = process.env.RABBIT_ADDRESS;
  ...
  consumir(fila, callback) {
    amqp.connect(this.host, (err, conn) => {
      ...
      conn.createChannel((err, ch) => {
        ...
        console.log(`[*] Observando a fila: ${fila} ${texto}! Para cancelar pressione CTRL+C`);
  ...
  transferir(filaOrigem, filaDestino) {
    amqp.connect(this.host, (err, conn) => {
      ...
      conn.createChannel((err, ch) => {
        ...
        console.log(`[*] Observando a fila: ${filaOrigem} ${texto}! Para cancelar pressione CTRL+C`);
