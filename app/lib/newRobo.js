...
class Requisicao {
  ...
  tratarCookie(cookies = []) {
    ...
    cookies.toString().split(/\s*,\s*/).forEach(function(pair) {
      pair = pair.split(/\s*=\s*/);
...
class Robo {
  constructor() {
    ...
    this.headers = {
      'User-Agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/51.0.2704.103 Safari/537.36', // Chrome,
    };
    this.cookies = {};
  }

  async acessar({url, method='GET', encoding='latin1', proxy=false, queryString={}, formData={}, json={}, headers={}, randomUserAgent=false, timeout=6000} = {}){
    if(!url || url === '') throw new Error('URL Vazia');

    if (randomUserAgent) this.setHeader({'User-Agent': this.requisicao.obterUserAgent()});
