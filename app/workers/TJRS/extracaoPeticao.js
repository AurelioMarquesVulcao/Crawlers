...
(async () => {
  ...
  const nomeFila = `${enums.tipoConsulta.Peticao}.${enums.nomesRobos.TJRS}.extracao`;
  ...
  new GerenciadorFila().consumir(nomeFila, async (ch, msg) => {
    ...
    let logger = new Logger('info', 'logs/PeticaoTJRS/PeticaoTJRSInfo.log', {
      nomeRobo: `${enums.tipoConsulta.Peticao}.${enums.nomesRobos.TJRS}`,
    ...
    try {
      ...
      if (resultadoExtracao.sucesso) {
        ...
        await Helper.feedbackDocumentos(resposta).catch((err) => {
          console.log(err);
          throw new Error(
            `PeticaoTJRS - Erro ao enviar resposta ao BigData - Processo: ${message.Processo}`
          );
        });
      } else {
